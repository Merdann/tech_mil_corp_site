...
async def create_element_admin(
    ...
    element = await DbQuery.create_element(db, tkn, CarouselAbout, data)
...
def get_all_items_admin(tkn: str, db: Session):
    Tkn.is_access_tkn_and_active_user_or_exc(tkn, db)
    all = DbQuery.get_all_items_admin(db, CarouselAbout)
    return all


def get_one_item_admin(item_id: UUID, tkn: str, db: Session):
    Tkn.is_access_tkn_and_active_user_or_exc(tkn, db)
    item = DbQuery.get_one_item_by_id_admin(db, item_id, CarouselAbout)
    return item
...
def delete_one_item_admin(item_id: UUID, tkn: str, db: Session):
    ...
    DbQuery.delete_one_item_by_id_admin(db, item_id, CarouselAbout)
